...
- Sources (each exposes `quarterly(api_id, metric, fye_month, years) -> {(cal_y,cal_q): value_local}`):
  ...
  - `FinMindSource` (TW) — `TaiwanStockFinancialStatements` (discrete quarters) + `TaiwanStockBalanceSheet`. Equity field is `Equity`. **Leading-cumulative repair (`_fix_leading_cumulative`):** pre-IPO / recently-listed filers whose first interim filing was the 半年報 are MISSING earlier quarters from FinMind, so the first available flow quarter of that year is a YEAR-TO-DATE CUMULATIVE (H1 in the Q2 slot), never de-cumulated — over-reporting ~2×. Detected data-drivenly (a present quarter whose immediately-preceding quarter of the same year is absent; no company list); quarters that follow a present quarter are already de-cumulated by FinMind and left alone. **REVENUE** is recovered from the independent discrete monthly dataset (`TaiwanStockMonthRevenue`, keyed on `revenue_year`/`revenue_month`) but ONLY when the monthly sum over the cumulative's span reconciles to the FS value within 0.5% (confirms parent≈consolidated) — else the quarter is DROPPED; missing earlier quarters in the span are back-filled from monthly too. **Other flow metrics** have no monthly proxy → the cumulative quarter is DROPPED (never a wrong value). Stock metrics untouched. Found & fixed: **6510** 精測 2015Q2 738→**408**, **6732** Sensortek 2019Q2 1,956→**1,240**, **6770** Powerchip 2020Q2 22,227→**11,515** / 2021Q2 28,574→**15,335** (NT$M) — all pre-listing years; the other 20 TW filers unchanged. Reconcile-or-drop proven on 6770 2019 (full-year cumulative, monthly parent diverges 25% → correctly dropped).
